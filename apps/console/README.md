...
The UI foundation is shadcn-owned components backed by Base UI primitives, Tailwind CSS v4 semantic tokens, and Lucide icons. The application shell uses the official shadcn Sidebar composition with desktop icon collapse and mobile off-canvas navigation. Keep business logic out of `src-tauri`; browser and desktop clients must share the React application and backend contracts.
