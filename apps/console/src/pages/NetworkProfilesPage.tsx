...
import { Check, Edit3, Globe2, Loader2, Plus, Radar, Save, ShieldCheck, Trash2 } from "lucide-react"
...
export function NetworkProfilesPage({ snapshot, dispatch, view = "profiles", onViewChange }: { snapshot: ControlPlaneSnapshot; dispatch: DispatchIntent; view?: string; onViewChange?: (view: string) => void }) {
  ...
  const [candidateAction, setCandidateAction] = useState<"approve" | "reject" | "register" | null>(null)
  ...
  function decideCandidate(candidate: ScanCandidateView, approve: boolean) {
    setCandidateAction(approve ? "approve" : "reject")
    ...
    }).finally(() => setCandidateAction(null))
  }
  ...
  return <>
    ...
    <Tabs value={view} onValueChange={onViewChange} className="mt-6"><TabsList className="h-auto flex-wrap rounded-none border border-border bg-background p-0" aria-label="Network profile views"><TabsTrigger value="profiles" className="rounded-none">Profiles</TabsTrigger><TabsTrigger value="scans" className="rounded-none">Discovery Scans</TabsTrigger><TabsTrigger value="candidates" className="rounded-none">Pending Candidates</TabsTrigger><TabsTrigger value="endpoints" className="rounded-none">Registered Endpoints</TabsTrigger><TabsTrigger value="provisioning" className="rounded-none">Device Provisioning</TabsTrigger><TabsTrigger value="history" className="rounded-none">History</TabsTrigger></TabsList>
      ...
      <TabsContent value="provisioning" className="mt-6">
        <Panel
          ...
          description="Discovery, Approval, Provisioning, and Registration are separate stages. Registration is never implied by Discovery."
        >
          <div className="mb-4 flex flex-wrap gap-2" aria-label="Device provisioning stages">
          ...
          {!readiness && !registration
            ? <EmptyState label="No Device Provisioning Evidence" detail="Use Control → Device Provisioning after Discovery and target confirmation, or verify readiness evidence here once a serial is confirmed." />
    ...
    <Sheet open={reviewCandidateId !== null} onOpenChange={(open) => { if (!open && !candidateAction) setReviewCandidateId(null) }}><SheetContent className="w-full rounded-none sm:max-w-md"><SheetHeader><SheetTitle>Candidate Review</SheetTitle><SheetDescription>Approval is required before the separate registration action is offered.</SheetDescription></SheetHeader>{reviewCandidate ? <div className="space-y-5 px-4 pb-4"><div className="border-y border-border py-4"><p className="font-medium">{reviewCandidate.host}:{reviewCandidate.port}</p><p className="drift-data mt-1 text-[10px] text-muted-foreground">{reviewCandidate.id} · {reviewCandidate.serial}</p><p className="mt-3 text-xs text-muted-foreground">{reviewCandidate.evidenceSummary}</p></div><div><FieldLabel htmlFor="candidate-reason">Decision Rationale</FieldLabel><textarea id="candidate-reason" value={candidateReason} onChange={(event) => setCandidateReason(event.target.value)} rows={4} className="mt-1 w-full rounded-none border border-input bg-background p-2 text-xs" placeholder="Bounded operator rationale" /></div>{candidateAction === null && feedback && !feedback.includes("approved") && !feedback.includes("registered") ? <p role="alert" className="border border-destructive p-3 text-xs text-destructive">{feedback}</p> : null}<div className="flex flex-wrap gap-2">{reviewCandidate.state === "pending_approval" ? <><Button onClick={() => decideCandidate(reviewCandidate, true)} disabled={candidateAction !== null}>{candidateAction === "approve" ? <Loader2 className="size-3.5 animate-spin" aria-hidden="true" /> : <Check className="size-3.5" aria-hidden="true" />}{candidateAction === "approve" ? "Approving…" : "Approve"}</Button><Button variant="outline" onClick={() => decideCandidate(reviewCandidate, false)} disabled={candidateAction !== null}>{candidateAction === "reject" ? <Loader2 className="size-3.5 animate-spin" aria-hidden="true" /> : <Edit3 className="size-3.5" aria-hidden="true" />}{candidateAction === "reject" ? "Rejecting…" : "Reject"}</Button></> : null}{reviewCandidate.state === "approved" ? <Button variant="secondary" onClick={() => registerCandidate(reviewCandidate)} disabled={candidateAction !== null} aria-busy={candidateAction === "register"}>{candidateAction === "register" ? <Loader2 className="size-3.5 animate-spin" aria-hidden="true" /> : <ShieldCheck className="size-3.5" aria-hidden="true" />}{candidateAction === "register" ? "Registering…" : "Register Candidate"}</Button> : null}</div>{candidateAction !== null ? <p role="status" aria-live="polite" className="text-xs text-muted-foreground">{candidateAction === "register" ? "Creating the device and endpoint records…" : candidateAction === "approve" ? "Recording approval…" : "Recording rejection…"}</p> : null}</div> : null}</SheetContent></Sheet>
  </>
}
