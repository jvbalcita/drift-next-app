...
export function NetworkProfilesPage({ snapshot, dispatch, view = "profiles", onViewChange }: { snapshot: ControlPlaneSnapshot; dispatch: DispatchIntent; view?: string; onViewChange?: (view: string) => void }) {
  ...
  return <>
    ...
    <Tabs value={view} onValueChange={onViewChange} className="mt-6"><TabsList aria-label="Network profile views"><TabsTrigger value="profiles">Profiles</TabsTrigger><TabsTrigger value="scans">Discovery Scans</TabsTrigger><TabsTrigger value="endpoints">Registered Endpoints</TabsTrigger></TabsList>
      ...
      <TabsContent value="endpoints" className="mt-6"><Panel title="Registered Endpoints" description="Each device's current endpoint. The endpoint is the mutable half of a device: the transport a device left stays as its own endpoint history, dated with when it was superseded, and is not listed or counted here."><EndpointTable endpoints={snapshot.endpoints} devices={snapshot.devices} /></Panel></TabsContent>
    </Tabs>
...
function SaveFailureNotice({ noticeRef, message }: { noticeRef: RefObject<HTMLDivElement | null>; message: string }) {
  return <div ref={noticeRef} tabIndex={-1} role="alert" aria-labelledby="profile-save-failure-title" className="flex items-start gap-2 border-l-2 border-destructive bg-destructive/10 p-3 text-xs text-foreground focus-visible:ring-3 focus-visible:ring-ring/50 sm:col-span-2"><AlertTriangle className="mt-0.5 size-4 shrink-0 text-destructive" aria-hidden="true" /><div><p id="profile-save-failure-title" className="font-semibold">Save failed. The profile was not saved.</p>{message ? <p className="mt-1">{message}</p> : null}</div></div>
}
