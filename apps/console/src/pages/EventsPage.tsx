...
import { DataTablePagination, EmptyState, FailureBadge, FormSelect, OperatorNotice, PageIntro, StatusBadge } from "./shared"
  ...
  const events = useMemo(
    () =>
      snapshot.events.filter((event) => {
        ...
        return (kind === "all" || event.kind === kind) && includes(event.actor, actor) && includes(`${event.resourceType} ${event.resourceId}`, resource) && includes(event.correlationId, correlation) && includes(event.occurredAt, time) && (severity === "all" || (severity === "normal" ? !event.failureClass : Boolean(event.failureClass)))
      }),
    ...
    <>
      ...
      <div className="flex flex-wrap items-end gap-3 border-y border-border py-3" aria-label="Event Filters">
        ...
        <FilterSelect
          ...
          value={severity}
          onChange={(next) => {
            setSeverity(next as "all" | "normal" | "failure")
            resetPage()
          }}
          options={[
      ...
      <Sheet open={Boolean(selected)} onOpenChange={(open) => !open && setSelectedId(null)}>
        <SheetContent className="w-full rounded-none sm:max-w-xl">
          <SheetHeader className="border-b border-border">
            <SheetTitle>{selected?.name ?? "Event Detail"}</SheetTitle>
            <SheetDescription>Sanitized, bounded event metadata.</SheetDescription>
          </SheetHeader>
          {selected ? (
            <dl className="grid gap-4 p-4 text-xs">
              <Detail label="Occurred" value={selected.occurredAt} />
              <Detail label="Actor" value={selected.actor} />
              <Detail label="Resource" value={`${selected.resourceType} / ${selected.resourceId}`} />
              <Detail label="Correlation ID" value={selected.correlationId} mono />
              <Detail label="Payload Summary" value={selected.payloadSummary} />
              <div>
                <dt className="text-muted-foreground">Failure Classification</dt>
                <dd className="mt-1">
                  <FailureBadge failureClass={selected.failureClass} />
                  {!selected.failureClass ? "None" : null}
                </dd>
              </div>
            </dl>
          ) : null}
...
function FilterSelect({ id, label, value, onChange, options }: { id: string; label: string; value: string; onChange: (value: string) => void; options: readonly { value: string; label: string }[] }) {
  return (
  ...
  return (
    <label htmlFor={id} className="text-xs font-semibold text-foreground">
      ...
      <span className="relative mt-1 block">
        <Search className="pointer-events-none absolute left-2 top-2.5 z-10 size-3.5 text-muted-foreground" aria-hidden="true" />
...
function EventTable({ events, onOpen }: { events: readonly EventView[]; onOpen: (id: string) => void }) {
  return (
    <ScrollArea className="h-[min(68vh,760px)] border border-border">
      <table className="w-full min-w-[940px] border-collapse text-left text-xs">
        <caption className="sr-only">Operational and audit event history</caption>
        <thead className="sticky top-0 bg-background">
          <tr className="border-b border-border text-[10px] tracking-[.08em] text-muted-foreground">
            ...
            <th className="p-3">Device or Resource</th>
...
function EventRow({ event, onOpen }: { event: EventView; onOpen: () => void }) {
  return (
    <tr className="border-b border-border/70 hover:bg-muted/50">
      <td className="p-3 text-muted-foreground">{event.occurredAt}</td>
      <td className="p-3">
        <button onClick={onOpen} className="text-left font-medium underline-offset-2 hover:underline focus-visible:outline-2 focus-visible:outline-primary">
          {event.name}
        </button>
      </td>
      <td className="p-3">
        <StatusBadge label={event.kind} tone={event.kind === "audit" ? "info" : "healthy"} />
      </td>
      <td className="p-3">{event.actor}</td>
      <td className="p-3">
        {event.resourceType} · {event.resourceId}
      </td>
      <td className="drift-data p-3 text-[10px]">{event.correlationId}</td>
      <td className="p-3">
        <FailureBadge failureClass={event.failureClass} />
        {!event.failureClass ? <span className="text-muted-foreground">Normal</span> : null}
      </td>
    </tr>
  )
}
