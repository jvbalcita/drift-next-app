...
export function OverviewPage({ snapshot, dispatch }: { snapshot: ControlPlaneSnapshot; dispatch: DispatchIntent }) {
  ...
  return <>
    ...
    <h2 className="sr-only">Device fleet</h2><Panel title="Device fleet" description={`${snapshot.devices.length} registered identities · transport state shown separately`} action={<div className="relative w-full sm:w-56"><Search className="pointer-events-none absolute left-2.5 top-1/2 size-3.5 -translate-y-1/2 text-muted-foreground" aria-hidden="true" /><input aria-label="Search devices" value={query} onChange={(event) => { setQuery(event.target.value); setPage(0) }} placeholder="Search devices" className="h-9 w-full rounded-none border border-input bg-background px-3 pl-8 text-xs outline-none transition focus:border-primary focus:ring-2 focus:ring-primary/20" /></div>}>
