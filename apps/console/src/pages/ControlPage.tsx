...
type ConsoleSettings = { gap: number; opacity: number; autoScreenOff: boolean; controlSmall: boolean; priority: "Speed" | "Quality"; controlsSide: "left" | "right"; workspaceSide: "left" | "right"; showTag: boolean; showIndex: boolean; showName: boolean; showIp: boolean }
...
const settingsDefaults: ConsoleSettings = { gap: 16, opacity: 100, autoScreenOff: false, controlSmall: false, priority: "Quality", controlsSide: "right", workspaceSide: "left", showTag: true, showIndex: true, showName: true, showIp: true }
...
export function ControlPage({ snapshot, dispatch, dispatchLab, labNotice = "", mirror }: { snapshot: ControlPlaneSnapshot; dispatch: DispatchIntent; dispatchLab?: (intent: ControlPlaneIntent) => Promise<MutationResult>; labNotice?: string; mirror?: LiveMirrorClient }) {
   ...
   */
  const sourceObservation = source ? snapshot.observations.find((candidate) => candidate.deviceId === source.id && candidate.freshnessToken)?.freshnessToken ?? "" : ""
  const sourceLease = source ? snapshot.leases.some((candidate) => candidate.deviceId === source.id && candidate.state === "active") : false
  const deviceModal = source ? <FloatingDevice device={source} followers={followers} workspace={workspace} settings={settings} position={position} pinned={modalPinned} onPinChange={setModalPinned} onPointerDown={beginDrag} onPointerMove={moveDrag} onPointerUp={endDrag} onClose={() => { void reportDispatch(dispatch, { type: "endDeviceControl", deviceId: source.id }, showToastMessage); setSourceId(null); setFollowerIds([]) }} onPreview={startPreview} onAction={handleDeviceAction} mirror={mirror} workspaceId={snapshot.workspaceId} observationToken={sourceObservation} hasLease={sourceLease} dispatch={dispatch} /> : null
...
function FloatingDevice({ device, followers, workspace, settings, position, pinned, onPinChange, onPointerDown, onPointerMove, onPointerUp, onClose, onPreview, onAction, mirror, workspaceId, observationToken, hasLease, dispatch }: { device: DeviceView; followers: readonly DeviceView[]; workspace: Workspace; settings: ConsoleSettings; position: FloatingPosition; pinned: boolean; onPinChange: (value: boolean) => void; onPointerDown: (event: PointerEvent<HTMLDivElement>) => void; onPointerMove: (event: PointerEvent<HTMLDivElement>) => void; onPointerUp: (event: PointerEvent<HTMLDivElement>) => void; onClose: () => void; onPreview: () => void; onAction: (value: string) => void; mirror?: LiveMirrorClient; workspaceId: string; observationToken: string; hasLease: boolean; dispatch: DispatchIntent }) {
  ...
  return <div className={`${pinned ? "relative z-20 self-start" : "fixed z-[100]"} flex items-start gap-3 ${controlsLeft ? "flex-row-reverse" : ""}`} style={positionStyle}>
    <div aria-label={`${device.displayName} floating phone frame`} className="flex shrink-0 flex-col overflow-hidden rounded-[22px] border-[3px] border-primary bg-slate-900 text-white" style={phoneFrameStyle}>
      <div className="flex h-8 shrink-0 items-center justify-between bg-slate-950 px-3 text-[9px]"><span>02:16 · {device.displayName}</span><span>{device.batteryPercent}%</span></div>
      <LiveMirrorSurface device={device} mirror={mirror} workspaceId={workspaceId} observationToken={observationToken} hasLease={hasLease} dispatch={dispatch} />
      <div className="flex shrink-0 items-center gap-2 border-t border-white/10 px-3 py-2">
        <p className="min-w-0 flex-1 text-[9px] leading-3 text-white/60">Preview mirrors the selected followers independently; it copies no commands to them.</p>
        <Button size="sm" variant="outline" disabled={followers.length === 0} onClick={onPreview}><MousePointer2 className="size-3.5" aria-hidden="true" />Start Preview</Button>
      </div>
    </div>
