...
describe("lab adapter client", () => {
  ...
  it("projects a lab status without inventing a preview", () => {
    ...
    expect(view).toMatchObject({ mode: "lab", readiness: "ready", observationLatencyMs: 412, confirmedSerial: "SERIAL1" })
  ...
  it("drops a truncated or mismatched preview and keeps a complete one", () => {
    const status = create(LabStatusSchema, { mode: LabMode.LAB, readiness: LabReadiness.READY, confirmedSerial: "SERIAL1" })
    const truncated = create(LabObservationBundleSchema, { serial: "SERIAL1", previewBase64: "aGk=", previewTruncated: true })
    const mismatched = create(LabObservationBundleSchema, { serial: "SERIAL2", previewBase64: "aGk=" })
    ...
    expect(toLabAdapterView(status, mismatched).lastScreenshotPreviewDataUrl).toBeUndefined()
  ...
  it("sends the local lab token only when one is configured", async () => {
    ...
    const fetchSpy = vi.spyOn(globalThis, "fetch").mockImplementation((input, init) => {
      ...
      return Promise.resolve(new Response(JSON.stringify({ status: toJson(LabStatusSchema, status) }), { status: 200, headers: { "content-type": "application/json" } }))
    })

    const options = { workspaceId: "workspace-1", requestId: "request-1", operatorId: "operator-1" }
    await createLabAdapterClient("http://127.0.0.1:8787", "lab-token-value")?.discoverLabDevices(options)
    await createLabAdapterClient("http://127.0.0.1:8787", "")?.discoverLabDevices(options)
  ...
  it("reports a refused lab call as a typed error instead of a silent fallback", async () => {
    ...
    await expect(client?.discoverLabDevices({ workspaceId: "workspace-1", requestId: "request-1", operatorId: "operator-1" })).rejects.toBeInstanceOf(LabAdapterRequestError)
    fetchSpy.mockRestore()
  })
})
