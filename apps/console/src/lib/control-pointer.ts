 ...
 * icon, all black, and the section and the pointer over a device are one thing
 * drawn one way.
 *
 ...
 * not one. It is filled and stroked black: the sidebar draws the same glyph as a
 * black outline against a light sidebar, and the same outline over a device's
 * screen at 24 px is a shape an operator cannot find - so the glyph is the
 * sidebar's and the rendering is solid black, which is what "all black" asks
 * for on a surface that is a photograph of a screen rather than a toolbar.
 */
...
export const controlPointerSvg = `<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path d="${controlPointerPath}" fill="#000000" stroke="#000000" stroke-linejoin="round"/></svg>`
