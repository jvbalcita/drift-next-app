        ...
        <SidebarInset className="min-w-0">
          <header className="flex h-16 shrink-0 items-center gap-2 border-b border-border transition-[width,height] ease-linear group-has-data-[collapsible=icon]/sidebar-wrapper:h-12">
            <div className="flex items-center gap-2 px-4">
              ...
              <Breadcrumb>
                <BreadcrumbList>
                  ...
                  <BreadcrumbItem><BreadcrumbPage>{activeSection}</BreadcrumbPage></BreadcrumbItem>
                </BreadcrumbList>
              </Breadcrumb>
            </div>
            <div className="ml-auto flex items-center gap-1 px-4">
              ...
              <Button variant="ghost" size="icon-sm" aria-label="Settings" onClick={() => setActiveSection("Settings")}><Settings2 className="size-4" aria-hidden="true" /></Button>
            </div>
          </header>
          <main className="drift-editorial-grid mx-auto w-full max-w-[1800px] flex-1 p-4 sm:p-6 lg:p-8">
            <Suspense fallback={<ConsoleLoadingState />}>
              {renderSection(activeSection, snapshot, dispatch)}
            </Suspense>
          </main>
        </SidebarInset>
      </SidebarProvider>
