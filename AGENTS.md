...
- Carry the coordinate frame with the coordinate. A device input that uses coordinates states the render size the device presents at — the `wm size` override, never the physical panel size and never a downscaled screenshot or vision frame — and the observation the coordinates were measured from; a coordinate without a frame, outside it, or bound to another observation is refused, never scaled. Cross-check that declared frame against the render size the device actually reports before dispatch: a frame the device does not present at is refused with a distinct error naming both sizes; a frame whose device size cannot be read, or whose reading is too old to refresh, is refused too (fail closed, never serve a stale size); and when a device reports both an override and a physical size, the override wins and the resolved value says which it came from.
