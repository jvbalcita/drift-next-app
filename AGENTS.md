...
- Admit an argument array to the device adapter's allow-list only as a fixed builder shape: a specific subcommand of a named device binary, with every variable position either a canonical bounded decimal or a name matching a strict allow-list pattern, and with the bounds re-derived inside the adapter rather than imported from the builder. No admitted position may accept whitespace, a shell metacharacter, a quote, a flag, a path or a second command, so an admitted array can never express command text.
...
- Admit operator-typed content through a boundary with no message at all: a bounded request body, read once into a buffer local to the call, handed straight to the component that owns it, with a maximum refused rather than truncated and nothing retained afterwards—not the body, not a copy, not on the handler, and not in a log. The answer names an opaque handle and the length and has no field the value could travel in, and that absence is asserted structurally rather than promised, because a value that never exists in a message field cannot be rendered by one.
- File a value registered by an operator under the workspace that registered it, and release it only into a request scoped to that workspace; a value that is not scoped to one is refused rather than released, and a release attempted in the wrong workspace is refused with the same sentence an unknown handle gets, so it neither consumes the value nor reveals that the handle exists.
