...
| P13 — one-device adapter spike | in progress | Boss authorized controlled one-lab-device P13 work on 2026-09-15. Implemented on `artisan/phase-13-real-device-vertical-slice` at `d8325e97a62a75b79bc2c1dbe59d22e070ad3c3e`: `internal/edge/adb`, `internal/edge/uiautomator`, `internal/edge/lab`, `proto/drift/v1/lab_adapter.proto`, console lab surfaces (Control layout preserved), ADR-0004 (native ADB + UIAutomator; helper **not** adopted; expansion **NO-GO**). Dual reviews GO at that SHA (Pass 1 architecture/security; Pass 2 QA/UI/evidence). Exit criteria not fully met: live matrix cells remain `pending operator serial confirmation` (~20 wireless transports attached; no inferred target). Opt-in `real_device_test.go` skips until `DRIFT_P13_LAB_MODE`/`DRIFT_P13_ADB_PATH`/`DRIFT_P13_LAB_SERIAL` are set. |
