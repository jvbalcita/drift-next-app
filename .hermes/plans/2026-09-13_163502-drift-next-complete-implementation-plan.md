...
| P13 — one-device adapter spike | in progress | Boss authorized controlled one-lab-device P13 work on 2026-09-15. Tip `3ac523c8ce10f4878c1f3673ab45379c2206c90d` on `artisan/phase-13-real-device-vertical-slice` (PR #19): `internal/edge/adb`, `internal/edge/uiautomator`, `internal/edge/lab`, `proto/drift/v1/lab_adapter.proto`, console lab surfaces (Control layout preserved), ADR-0004 (native ADB + UIAutomator; helper **not** adopted; expansion **NO-GO**). Dual reviews GO; CI green (Frontend/Go/Contract/Hygiene/Tauri) at tip. Exit criteria not fully met: live matrix cells remain `pending operator serial confirmation` (~20 wireless transports attached; `DRIFT_P13_LAB_SERIAL` unset; no inferred target). Opt-in `real_device_test.go` skips until `DRIFT_P13_LAB_MODE`/`DRIFT_P13_ADB_PATH`/`DRIFT_P13_LAB_SERIAL` are set. |
