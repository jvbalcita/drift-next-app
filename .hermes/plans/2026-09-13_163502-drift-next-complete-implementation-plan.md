...
| P14 — registration/runtime spool | in progress | Boss authorized controlled one-lab-device P14 registration + optional runtime spool (USB/wireless provisioning path) on 2026-09-15 on branch `artisan/phase-14-registration-runtime-spool` (base `84564f8` / merged P13 main). No fleet, multi-device, mirrored, production, or unattended work. Implementation in progress: `internal/edge/spool`, `internal/edge/connection` session reconnect/indeterminate, `internal/edge/registration`, `internal/discovery` authorized lab scanner, console provisioning/runtime/spool flows, `docs/operations/edge-recovery.md`. Validation, dual reviews, and merge pending. |
