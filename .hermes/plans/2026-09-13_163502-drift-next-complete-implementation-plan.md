...
| P13 — one-device adapter spike | in progress | Boss authorized controlled one-lab-device P13 work on 2026-09-15. A read-only observation vertical slice is implemented on branch `artisan/phase-13-real-device-vertical-slice`: `internal/edge/adb`, `internal/edge/uiautomator`, `internal/edge/lab`, `proto/drift/v1/lab_adapter.proto`, and the console lab-adapter surfaces, with ADR-0004 recording the adapter decision (native ADB + built-in UIAutomator dump; optional helper **not** adopted). Exit criteria are **not** met: all real-device cells in `tests/compatibility/adb/matrix.md` remain `pending operator serial confirmation` because the lab host enumerates 20 attached wireless transports and P13 forbids inferring a target; the opt-in test `internal/edge/lab/real_device_test.go` skips until `DRIFT_P13_LAB_MODE`/`DRIFT_P13_ADB_PATH`/`DRIFT_P13_LAB_SERIAL` are set. Dual reviews and a merged PR are still outstanding. Expansion beyond one lab device is **NO-GO**. |
...
- **P13 authorization gate:** satisfied for controlled one-device work. Boss explicitly authorized a controlled, lab-only, one-device P13 test on 2026-09-15, and implementation proceeds on branch `artisan/phase-13-real-device-vertical-slice`. The authorization covers **one confirmed lab device, read-only observation, attended** only; it does not cover production devices or accounts, a fleet, multi-device or mirrored execution, unattended runs, wireless-debugging enablement, helper installation, or any P14 registration/port-provisioning work. Sentinel/security review of the adapter boundary is still required before merge, and expansion beyond one lab device remains **NO-GO** until additional owner authorization, single-target confirmation UX proven in operations, and Sentinel review of the specific expansion.
