...
| P10.5 — Android interaction recorder and skill compiler | complete | Fake-only recorder sessions, grouped logical events, sanitized BEFORE/ACTION/AFTER evidence, explicit review/redaction, immutable typed skill promotion, shared-brain provenance, and deterministic semantic replay were squash-merged via PR #15 as `cbc298f`; real Android/ADB remains gated behind P13. |
| P11 — typed console integration | complete | Browser-only typed control-plane client and deterministic mock transport, Control preview, device/network/group/agent/run/event/settings/policy surfaces, optimistic-concurrency feedback, independent target outcomes, accessible responsive states, and reduced-motion support were squash-merged via [PR #16](https://github.com/jvbalcita/drift-next-app/pull/16) as `4b5a824`. All five GitHub checks and post-merge local typecheck, lint, test, build, Go, Buf, generated-artifact, security, and diff gates passed; lint retains five pre-existing warnings and Vite retains its main-chunk advisory. |
| P12 — accounts/settings/policy UX | complete | Metadata-only account sources/references, service and run projections with append-only history, explicit account-device assignments, disabled connector audit outcomes, typed scoped settings, immutable policy versions, auditable decisions, redaction checks, and browser-only console surfaces were implemented on `a63bebb` and submitted via [PR #17](https://github.com/jvbalcita/drift-next-app/pull/17). All five GitHub checks and local Go, race, frontend, Buf, generated-artifact, security, and diff gates passed; no external connector, credential, device, ADB, or shell path was introduced. |
| P13 — one-device adapter spike | complete | Boss authorized controlled one-lab-device P13 work on 2026-09-15. Tip `129bf53` on `artisan/phase-13-real-device-vertical-slice` (PR #19): real ADB + UIAutomator adapters, typed lab service, Connect transport, console lab surfaces (Control layout preserved), ADR-0004 (native path; helper **not** adopted; expansion **NO-GO**). Operator confirmed `192.168.1.109:5555`; `TestRealDeviceReadOnlyObservation` **PASS**; matrix + `evidence-2026-09-15.md` measured. Dual independent reviews GO at evidence tip `8824493` (Security Review GO; Bugbot no findings); CI green (Frontend/Go/Contract/Hygiene/Tauri). Remaining honest gaps: USB path, forced timeout, transport-id change, dump file-fallback — listed in matrix, do not block P13 exit. |
| P14 — registration/runtime spool | complete | Squash-merged via [PR #20](https://github.com/jvbalcita/drift-next-app/pull/20) as `cba37a9`. Controlled one-lab-device path: probe-backed `LabRegistrationService` (no client prerequisite booleans), durable Approve→Register in migration `0017` + SQLite store with memory hydrate/rollback, bounded in-process spool, connection session observations, authorized CIDR/USB lab scanner, Control-page UI, `docs/operations/edge-recovery.md`. Dual reviews GO (Security + Bugbot) on tip `fd1ccba`; all five CI checks green. No fleet/production/unattended/second DB. |
| P15 — artifacts/media transport | complete | Merged via [PR #21](https://github.com/jvbalcita/drift-next-app/pull/21) as `5bd872e` (reviewed tip `30a4225`). Local CAS under app data + SQLite metadata, fail-closed admission, lab capture persist isolation, Connect artifact RPCs, console Artifacts UI, `docs/operations/artifacts-and-retention.md`. Dual reviews GO (Security + Bugbot) on final tip; all five CI checks green. No remote object storage/SFU/TURN/mTLS/production identity. |
| P16 — production hardening | in progress | Next phase after P15 merge. OIDC/RBAC/mTLS/observability/backup production hardening only after local runtime risks are measured; no implementation started on this tip. |
...
- **P10.5:** fake-only recorder/skill behavior was implemented at `6e73ea5` with replay-policy enforcement at `ff9332d`, squash-merged via PR #15 as `cbc298f`; all five CI checks and local Go, race, frontend, Buf, generated-artifact, security, and diff gates passed. The phase branch was removed locally and remotely.
- **P11:** typed console integration was implemented at `f909cee` with mock safety/invariant follow-up at `d316f23`, squash-merged via PR #16 as `4b5a824`; all five CI checks and local typecheck, lint, test, build, Go, Buf, generated-artifact, security, and diff gates passed. The phase branch was removed locally and remotely. The console remains browser-only and mock-backed; no ADB, device library, credential, or production integration was introduced.
- **P12:** bounded account sources/references, service and run history, explicit account-device assignments, disabled connector outcomes, typed scoped settings, immutable policy versions, policy decision visibility, redaction tests, and Accounts/Policies/Settings console surfaces were implemented on `a63bebb` and opened as PR #17. All five GitHub checks and local Go, SQLite race, frontend, Buf, generated-artifact, security, and diff gates passed; the phase remains browser-only and connector-disabled.
- **P13 authorization gate:** satisfied and merged (PR #19). Controlled one-lab-device read-only observation completed with live evidence and dual GO.
- **P14 authorization gate:** satisfied and merged (PR #20 as `cba37a9`). Controlled one-lab-device registration + optional in-process spool completed with Connect/proto/SQLite durable Approve→Register, dual GO, and green CI. Expansion beyond one lab device remains **NO-GO**.
- **P15 execution gate:** satisfied and merged (PR #21 as `5bd872e`). Local CAS + SQLite metadata, fail-closed admission, authorized/audited artifact access, lab capture isolation, and console Artifacts UI completed with dual GO on tip `30a4225` and green CI. Remote object storage, SFU, TURN, and production identity remain deferred.
- **P16 execution gate:** phase is now **in progress**. Production hardening may begin from the merged local runtime; no fleet or remote media infrastructure is authorized by this note.
