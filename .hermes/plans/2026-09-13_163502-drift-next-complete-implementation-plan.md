...
| P13 — one-device adapter spike | in progress | Boss authorized controlled one-lab-device P13 work on 2026-09-15. Branch `artisan/phase-13-real-device-vertical-slice` (PR #19): adapters + lab service + Connect + console lab surfaces (Control layout preserved) + ADR-0004 (native path; helper **not** adopted; expansion **NO-GO**). Operator confirmed serial `192.168.1.109:5555`; opt-in `TestRealDeviceReadOnlyObservation` **PASS**; live matrix + `tests/compatibility/adb/evidence-2026-09-15.md` filled with measured results (Android 12 / API 31, TCP). Dual reviews GO pending re-check on evidence tip; CI green on prior tip. |
