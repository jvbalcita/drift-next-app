...
| P14 — registration/runtime spool | in progress | Boss authorized controlled one-lab-device P14 on 2026-09-15 (`artisan/phase-14-registration-runtime-spool`, PR #20). Delivered Go kernels (spool, session, probe-backed registration+Approve, authorized lab scanner), console mock flows, `docs/operations/edge-recovery.md`. Local gates green. Dual reviews: WIP merge GO pending Bugbot tip; **P14 complete exit still requires** Connect/proto/SQLite wiring of server-side `PrerequisiteProbe` + durable Approve→Register (not client booleans). No fleet/production/unattended. Tip SHA tracked on PR head. |
...
- **P14 authorization gate:** satisfied for controlled one-device registration/runtime-spool work. Boss explicitly authorized P14 controlled registration and optional edge-runtime spool for **one confirmed lab device**, including the approved USB/wireless provisioning path, on 2026-09-15 on branch `artisan/phase-14-registration-runtime-spool`. The authorization does **not** cover fleet rollout, multi-device or mirrored execution, production devices/accounts, or unattended execution. Expansion beyond one lab device remains **NO-GO**.
