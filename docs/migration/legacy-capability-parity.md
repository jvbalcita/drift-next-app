...
| Device lifecycle state (registered/active/unavailable/retired) | `src/registry/state-service.ts:69` `DeviceStateService` | **Ruled out, not ported.** A device has no lifecycle beyond its identity and its observation history: the state machine (`internal/devices/model.go` `CanTransition`/`Transition`) is parked unreachable with the reason recorded beside it, and the wire status is derived from observation facts (`internal/transport/connect/device.go` `deviceStatusProto`) rather than from the persisted `devices.state` column. `proto/drift/v1/device.proto` `DeviceStatus` survives as an enum; no lifecycle is advanced through it | DELIBERATELY DROPPED |
...
| Fleet monitor — heartbeats, stale detection, fleet health | `src/orchestrator/monitor.ts:31-135` (`onHeartbeat`, `detectStale`, `getFleetHealth`) | None; there is no heartbeat sampling, no stale detection and no fleet rollup. A device's status is now derived from observation facts (`internal/transport/connect/device.go` `deviceStatusProto` reads the current endpoint observation and the last positive observation, and no longer reads `devices.state`), which is reachability of a kind — but nothing samples, nothing ages a reading into a fleet health view, and there is no health RPC | MISSING |
...
| Device list with live status | `routes.ts:493` `GET /devices`; `dashboard/src/pages/MonitorPage.tsx` filters and group collapse | `DeviceService.ListDevices`; `DevicesPage.tsx`. Status is derived from observation facts - a device reads online while it has a current endpoint observation and has not been observed leaving - and no longer from the persisted lifecycle column; a device nobody has observed is not reported online. `battery_percent`/`latency_ms` are never populated | PARTIAL |
...
4. **Fleet monitoring and health.** Health sampling, the sampling loop, heartbeat/stale detection and the fleet-health rollup have no producer or consumer in Drift Next: `HealthService.Record` has no callers, there is no health RPC, and nothing ages a reading into a fleet view. What is no longer true is the reachability half: a device's status is derived from observation facts rather than from the persisted lifecycle column, and a device that leaves is recorded as having left, so the registry stops reporting a device as online once nothing observes it. There is still no fleet health rollup and no sampling loop.
