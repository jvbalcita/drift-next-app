...
- The assertions this record rests on were proved to bite by injecting faults against the committed tree (`e098424`, the implementation commit) and reverting each one:
  - **1 — the boundary logs the value.** `log.Printf("event=text_reference_registered handle=%s value=%s", handle, value)` added to `ServeHTTP`. The end-to-end test failed with `the value was rendered into the process loggers` — and reported only which sink leaked, never the value.
  - **2 — the payload gains a field the value could travel in.** `Value string` added to `TextReferenceRegistration`. The structural test failed with `payload fields = 3, want 2: every additional field is somewhere the value could travel`.
  - **3 — the registry forgets the workspace.** `Register` and `Resolve` keyed by the handle alone. Both the registry test and the end-to-end test failed with `another workspace released a value it did not register`.
  - **4 — the boundary renders the registry's error text.** `refuse(writer, http.StatusConflict, err.Error())` in place of the fixed sentence. The hostile-registrar test failed with `code "conflict": the answer rendered the registry's message, which carried the value`.

  All four were reverted with `git checkout --` on the committed files, `git status --porcelain` was confirmed empty afterwards, and `go test ./...` was green again on the restored tree. A green run alone is not evidence that an assertion bites; these four injections are.
