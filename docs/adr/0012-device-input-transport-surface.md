...
- The refusal mapping's tripwire is **not yet recorded**: it is the next commit on this branch, and this record does not claim it. A fault must be injected into the mapping, the vocabulary test must fail on it, the fault must be reverted, and its absence must be shown by `git diff` rather than asserted. The green run alone is not evidence that the assertions bite.
