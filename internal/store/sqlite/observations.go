...
func (s *ObservationService) Record(ctx context.Context, observation observations.ObservationSnapshot, actorType, actorID string) error {
	...
	return WithTx(ctx, s.store.db, func(tx *sql.Tx) error {
		...
		if _, err := tx.ExecContext(ctx, `INSERT INTO observation_snapshots (id, workspace_id, device_id, captured_at, capture_correlation_id, coordinate_space, package_name, activity_name, app_version, orientation, display_width, display_height, ui_tree_hash, screenshot_hash, source, protocol_version, capture_status, error_class, state, created_at, model_version, truncated, freshness_token, capture_skew_ms) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, observation.ID, observation.Workspace, observation.DeviceID, observation.CapturedAt.UTC().Format(time.RFC3339Nano), observation.CaptureCorrelationID, observation.CoordinateSpace, nullableString(observation.PackageName), nullableString(observation.ActivityName), nullableString(observation.AppVersion), nullableString(observation.Orientation), observation.DisplayWidth, observation.DisplayHeight, nullableString(observation.UITreeHash), nullableString(observation.ScreenshotHash), observation.Source, observation.ProtocolVersion, observation.CaptureStatus, errorClass, observations.Recorded, now, observation.ModelVersion, boolInt(observation.Truncated), observation.FreshnessToken, observation.CaptureSkewMillis); err != nil {
...
const observationQuery = `SELECT id, workspace_id, device_id, captured_at, capture_correlation_id, coordinate_space, package_name, activity_name, app_version, orientation, display_width, display_height, ui_tree_hash, screenshot_hash, source, protocol_version, capture_status, error_class, state, model_version, truncated, freshness_token, capture_skew_ms FROM observation_snapshots`
...
func scanObservation(row interface{ Scan(...any) error }, observation *observations.ObservationSnapshot) error {
	...
	var packageName, activity, appVersion, orientation, uiTree, screenshot, errorClass sql.NullString
	var truncated int
	err := row.Scan(&observation.ID, &observation.Workspace, &observation.DeviceID, &captured, &observation.CaptureCorrelationID, &observation.CoordinateSpace, &packageName, &activity, &appVersion, &orientation, &observation.DisplayWidth, &observation.DisplayHeight, &uiTree, &screenshot, &observation.Source, &observation.ProtocolVersion, &observation.CaptureStatus, &errorClass, &state, &model, &truncated, &freshness, &observation.CaptureSkewMillis)
	if err != nil {
		return err
	}
	...
	if appVersion.Valid {
		observation.AppVersion = appVersion.String
	}
